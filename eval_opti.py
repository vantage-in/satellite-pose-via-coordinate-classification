...
class SimCCPoseRefiner:
    ...
    def prepare_pdf_and_weights(self, logits_x, logits_y, sigma=2.0):
        """
        Logits로부터 PDF(Spline)를 만들고, 동시에 Confidence Weight를 계산하여 저장함.
        """
        self.splines_x = []
        self.splines_y = []
        self.weights = []
        
        # 1. Confidence Weight 계산 (Soft Logic)        
        # 각 축의 Max Probability (Confidence)
        ...
        # 제안하신 Score: 둘 중 작은 값을 해당 점의 대표 신뢰도로 사용
        # shape: (N_kpts,)
        self.weights = np.minimum(conf_x, conf_y)
        
        # (옵션) 가중치 강화: 신뢰도가 높은 것에 더 가중치를 주고 싶다면 제곱 등을 할 수 있음
        ...
        for i in range(n_kpts):
            # Softmax & Smoothing for PDF
            # (Weight 계산용 Prob과 별개로 Smoothing된 분포를 최적화에 사용)
    ...
    def objective_function(self, params, points_3d, crop_meta):
        """
        Weighted Negative Log-Likelihood Minimization
        Loss = Sum( Weight_i * NLL_i )
        """
        rvec, tvec = params[:3], params[3:]
        ...
        for i in range(len(self.splines_x)):
            ux, uy = simcc_pts_x[i], simcc_pts_y[i]
            w = self.weights[i] # [NEW] 현재 Keypoint의 가중치
            
            # --- X축 NLL ---
            if not (0 <= ux < n_bins_x):
                # 범위 밖이면 페널티 부여 (가중치 w 곱해서 영향력 조절)
                val_x = -(penalty_base + abs(ux - n_bins_x/2)*0.1)
            else:
                # Spline 값은 Log-Prob (음수일 수도 있음)
            ...
            # => Minimize Weighted Negative Sum
            # Loss += -1 * Weight * (LogProbX + LogProbY)
...
def main():
    ...
    points_3d = load_tango_3d_keypoints(MODEL_3D_POINTS_FILE) # (N_all, 3)
    K, dist = load_camera_intrinsics(CAMERA_FILE)
    ...
    for item in tqdm(pose_list):
        ...
        #     logits_x, logits_y, threshold=0.1, min_kpts=6
        # )
