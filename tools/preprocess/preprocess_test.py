...
def main():
    ...
    def process_test_split(subset_name):
        subset_dir = os.path.join(speedplus_dir, subset_name)
        img_source_dir = os.path.join(subset_dir, 'images')
        mask_source_dir = os.path.join(subset_dir, 'masks') # BBox
        
        json_filename = 'test.json' 
        json_path = os.path.join(subset_dir, json_filename)
        
        output_img_dir = os.path.join(speedplus_dir, f'{subset_name}_preprocessed')
        os.makedirs(output_img_dir, exist_ok=True)

        print(f"\n--- Processing Test Set: {subset_name} ---")
        print(f" Source: {subset_dir}")
        print(f" Target: {output_img_dir}")
        ...
        for image_id_counter, image_data in enumerate(tqdm(labels), 1):
            ...
            mask_path = os.path.join(mask_source_dir, image_name)
            image_path = os.path.join(img_source_dir, image_name)
            ...
            keypts2d_orig = keypts2d_orig_raw.T
            keypts3d_cam_vis = keypts3d_cam.T
            ...
            if num_keypoints_count > 0:
                ...
                bbox_coco = [b_xmin, b_ymin, b_xmax - b_xmin, b_ymax - b_ymin]
            else:
                bbox_coco = [0, 0, 0, 0]
        ...
        # Save as a json file
        output_json_name = f'test_{subset_name}.json'
        output_json_path = os.path.join(annot_out_dir, output_json_name)
        with open(output_json_path, 'w') as f:
            json.dump(coco_output, f, indent=4)
        print(f"Saved annotation file to {output_json_path}")
        print(f"Total {len(coco_output['images'])} images processed for {subset_name}.")

    for subset in test_subsets:
        process_test_split(subset)
